...
from .server import PEEPpacket #, RequestToBuy, RequestItem, SendItem, RequestMoney, SendMoney, FinishTransaction
from playground.network.packet import PacketType
from playground.network.packet.fieldtypes import UINT32, STRING, UINT16, UINT8, BUFFER, BOOL, INT, ListFieldType
        ...
        for packet in self.deserializer.nextPackets():
            ...
            if self._state == 1 and packet.Type == 1:
                ...
                if checkvalue:
                    ...
                    print("RIP-ACK Received from Server. Ack. No.:", packet.Acknowledgement, "Closing down the connection.")
                    print("Current active timers after receiving RIP-ACK: ", self.t)
                    self.pop_sending_window(packet.Acknowledgement)
                    exc=0
    ...
    def encapsulating_packet(self,data_from_BL_1):
        ...
        if chunk == b'rip':
            if self.called_once == 0:
                ...
                print("Sent the RIP to Server with Seq. No.:", self.rip.SequenceNumber, "Ack. No.:", self.rip.Acknowledgement)
                ...
                print("Current active timers after sending RIP: ", self.t)
            else:
                self.rip = PEEPpacket()
                self.rip.Type = 3
                self.rip.Acknowledgement = self.global_number_ack
...
'''if __name__ == "__main__":
    ...
    Clientfactory = StackingProtocolFactory(lambda: PEEPClient(loop), lambda: ShopClientProtocol(loop))
    ptConnector = playground.Connector(protocolStack=Clientfactory)
